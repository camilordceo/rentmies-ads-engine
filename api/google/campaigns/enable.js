/**
 * POST /api/google/campaigns/enable?id=<rentmies_id>
 *   Activates a paused campaign (status=ENABLED).
 */
...
module.exports = async (req, res) => {
  ...
  const { data: row } = await sb.from('google_campaigns')
    .select('id, customer_id, campaign_resource_name')
  ...
  try {
    ...
    await axios.post(url, {
      operations: [{ updateMask: 'status', update: { resourceName: row.campaign_resource_name, status: 'ENABLED' } }]
    }, { headers: authHeaders(token), timeout: 20000 })

    await sb.from('google_campaigns').update({ status: 'ENABLED', updated_at: new Date().toISOString() }).eq('id', id)
    return res.json({ ok: true, id, status: 'ENABLED' })
  } catch (err) {
    const detail = err.response?.data?.error?.message || err.message
    return res.status(500).json({ error: 'Enable failed', detail })
  }
}
