/**
 * RENTMIES — Meta Ads Publisher (lib)
 *
 * Crea Campaign → Ad Set → Ad Creative → Ad en Meta Marketing API v21.0.
 * Objetivo: OUTCOME_TRAFFIC (clicks a WhatsApp).
 * Usado por api/meta-ads.js.
 */
...
async function createAdSet({ adAccountId, accessToken, campaignId, name, dailyBudget, ciudad, duracion = 14 }) {
  const cityNorm = (ciudad || 'bogota').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, '')
  const geo      = CITY_GEO[cityNorm] || CITY_GEO['bogota']
...
/**
 * Crea una campaña completa con todos los ads.
 *
 * @returns {{ campaignId, adSetId, ads: [{adId, adSetId, creativeId, status, headline}] }}
 */
async function createCampaign({ adAccountId, accessToken, pageId, inmueble, ciudad, dailyBudget, duracion, ads, ctaType, ctaUrl }) {
  ...
  return { campaignId: campaign.id, adSetId: adSet.id, ads: createdAds }
}

module.exports = { createCampaign, createMetaCampaign, createAdSet, createAdCreative, createAd, CITY_GEO, CTA_TYPE_MAP }
